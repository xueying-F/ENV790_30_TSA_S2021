...
for(EnergyType in 1:ncoln){
  Detrend_data <- MonthlyData_subset[,(EnergyType+1)]-(beta0+beta1*n)
  str(Detrend_data)
  
  #change Detrend_data to TS
  Detrend_data_ts <- ts(Detrend_data, frequency = 12) 
  
  #plot ACF and PACF for Detrend_data_ts
  par(mfrow=c(1,2)) 
  Acf(Detrend_data_ts,lag.max=40,main=paste("AFC of ",colnames(MonthlyData_subset)[(1+i)],sep="")) 
  Pacf(Detrend_data_ts,lag.max=40,main=paste("PAFC of ",colnames(MonthlyData_subset)[(1+i)],sep=""))
}
...
ggplot(MonthlyData_subset, aes(x=Date, y=MonthlyData_subset[,(1+EnergyType)])) +
            geom_line(color="blue") +
            ylab(paste0(colnames(MonthlyData_subset)[(1+EnergyType)]," (Trillion Btu)",sep="")) +
