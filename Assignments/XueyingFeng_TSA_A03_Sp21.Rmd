...
>For first two plot, ACF with significant correlations on whole lags, which means they are highly correlated with each other. Also, the slow decrease in the ACF as the lags increase is due to the trend, while the “scalloped” shape is due the seasonality.
>However,for third plot,there are non-significant autocorrelation for some lags, but in general, the ACF shows an oscillation, which indicates seasonal series, but no trend. The peaks occur at lags of 12, 24, 36 months.
...
```
>Detrend plot makes a time series stationary, which does not have trend and are not dependent on time.
...
```
>The plot change a little bit, but not too much.
...
```
>First plot has more obvious oscillations
>The first two plots imply that the role of the independent variables isn’t affected by seasonal patterns, but for the other two, the overall movement of the time series with seasonal effect smoothed out. 
...
```
>The spike value of ACF in plot 1 and 2 are different from that of plots from Q1.
> For Thrid plot, the slow decrease in the ACF as the lags increase is due to the trend, while the “scalloped” shape is due the seasonality, which is total different trend from Q1.
