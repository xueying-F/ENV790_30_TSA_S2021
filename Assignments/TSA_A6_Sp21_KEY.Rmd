...
#But Note that there is a slow decay so we might need differencing. 
...
```{r}
# To identify the SARIMA model order let's look at ACF/PACF plots again from Q1. It seems we are dealing with a AR model of order 2 and since we have a trend we need differencing. (See seasonal ACF and PACF)
...
nsdiffs(ts_ng)
## output: 1
diff_ts_ng <- diff(ts_ng, differences = 1, lag = 12)
...
SARIMA_fit <- Arima(ts_ng,order=c(1,0,1),seasonal = c(0,1,1),include.drift = TRUE)
print(SARIMA_fit)
