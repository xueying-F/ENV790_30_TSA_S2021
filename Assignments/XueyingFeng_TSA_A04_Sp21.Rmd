...
#Comparing ACFs & PACFs
#Biomass
par(mar=c(3,3,3,0));par(mfrow=c(2,2))
Acf(df_MonthlyData_full$Biomass,lag.max=40,main="Biomass ACF",ylim=c(-.2,1))
Acf(df_MonthlyData_full$BioDiff,lag.max=40,main="Diff Biomass ACF",ylim=c(-.2,1))
Pacf(df_MonthlyData_full$Biomass,lag.max=40,main="Biomass PACF",ylim=c(-.2,1))
Pacf(df_MonthlyData_full$BioDiff,lag.max=40,main="Diff Biomass PACF",ylim=c(-.2,1))

# RenewDiff
par(mar=c(3,3,3,0));par(mfrow=c(2,2))
Acf(df_MonthlyData_full$Renewable,lag.max=40,main="Renewable ACF",ylim=c(-.2,1))
Acf(df_MonthlyData_full$RenewDiff,lag.max=40,main="Diff Renewable ACF",ylim=c(-.2,1))
Pacf(df_MonthlyData_full$Renewable,lag.max=40,main="Renewable PACF",ylim=c(-.2,1))
Pacf(df_MonthlyData_full$RenewDiff,lag.max=40,main="Diff Renewable PACF",ylim=c(-.2,1))

# RenewDiff
par(mar=c(3,3,3,0));par(mfrow=c(2,2))
Acf(df_MonthlyData_full$Hydroelectric,lag.max=40,main="Hydroelectric ACF",ylim=c(-.2,1))
Acf(df_MonthlyData_full$HydroDiff,lag.max=40,main="Diff Hydroelectric ACF",ylim=c(-.2,1))
Pacf(df_MonthlyData_full$Hydroelectric,lag.max=40,main="Hydroelectric PACF",ylim=c(-.2,1))
Pacf(df_MonthlyData_full$HydroDiff,lag.max=40,main="Diff Hydroelectric PACF",ylim=c(-.2,1))
...
# Bio_SMKtest <- SeasonalMannKendall(ts_MonthlyData_full[,1])
# print("Results for Seasonal Mann Kendall OF Biomass")
# print(summary(Bio_SMKtest))
...
for(ColNum in 1:6){
  ...
  SpCor_MonthlyData=cor(MonthlyData_full_yearly,Year,method="spearman")
...
MonthlyData_subset_ts2 <- ts(MonthlyData_subset_new[,2:3], frequency = 12, start = c(1984,1,1),end = c(2020,10,1)) 
...
```
>The trend component of both of them is an increasing pattern. The random component is a regularly repeating pattern before 2015 in Solar dataset and before 2010 in Wind dataset.So there are some seasonality on that.
...
plot(decompose_Wind_ts2)
```
>The random outputs are different from "additive" ones. The regularly repeating pattern start from 1990 for Solar, and Start from 2000 in Wind.
