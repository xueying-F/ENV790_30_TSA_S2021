...
Rainfall_ts <- ts(BR_complete$Rainfall, frequency = 12, start = c(1901, 1, 1), end = c(2016, 12, 1)) 
str(Rainfall_ts)

Temperature_C_ts <- ts(BR_complete$Temperature_C, frequency = 12, start = c(1901, 1, 1), end = c(2016, 12, 1)) 
str(Temperature_C_ts)
...
* `stats::stl()`: decompose a time series into seasonal, trend and irregular components using loss (locally weighted scatter plot smoothing). Need to specify `s.window = "period"`. That will default seasonal period to be equal frequency established in ts().
...
Rainfall_decompose <- stl(Rainfall_ts, s.window = "period")
Temperature_C_decompose <- stl(Temperature_C_ts, s.window = "period")
...
Rainfall_decompose_df <- data.frame(Rainfall_decompose$time.series)
Temperature_C_decompose_df <- data.frame(Temperature_C_decompose$time.series)


# Exercise 6: Visualize how the trend maps onto the data for both series
par(mfrow=c(1,2))
plot(Rainfall_decompose_df$trend, type = "l")


plot(Rainfall_decompose_df)
plot(Temperature_C_decompose)
...
SARIMA_Temp <- auto.arima(Temperature_C_ts)
summary(SARIMA_Temp)
...
SARIMA_Temp_Exo <- auto.arima(Temperature_C_ts, xreg= Rainfall_ts)
