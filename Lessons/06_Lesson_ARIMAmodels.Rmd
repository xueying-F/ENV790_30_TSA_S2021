...
#Acf & Pacf from package forecast
#acf & pacf from package stats
```
...
decompose_residential_price <- decompose(ts_electricity_price[,"Residential"],"additive")
  ##可加性分解（additive）和可乘性分解（multiplicative）两种
...
deseasonal_residential_price <- seasadj(decompose_residential_price)  ## output is time series object

df_residential <- data.frame( Month = electricity_price_processed$Month,
                             Residential = electricity_price_processed$Residential,
                             NonSeasonalResidential = as.numeric(deseasonal_residential_price)) ## change to numeric
...
#Note that p-value greater then 0.05 so we accept H0. Data has stochastic trend(H0)
